...
namespace imdraw {
	...
	})) 
	{
		...
		attributes.at(locations.at("position")) = { pos_vbo, 3 };

		if (geo.uvs) {
			auto uv_vbo = imdraw::make_vbo(geo.uvs.value());
			attributes.at(locations.at("uv")) = { uv_vbo,2 };
		}

		if (geo.normals) {
			auto normal_vbo = imdraw::make_vbo(geo.normals.value());
			attributes.at(locations.at("normal")) = { normal_vbo,3 };
		}

		if (geo.colors) {
			auto color_vbo = imdraw::make_vbo(geo.colors.value());
			attributes.at(locations.at("color")) = { color_vbo,3 };
		}
